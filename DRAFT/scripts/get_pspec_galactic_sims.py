...
if testing or not local:
    ...
    if testing:
        ...
        plfolder = 'reports/galactic_sims/maps_masks/'
        ...
        savefig('%s/masks.pdf' %(plfolder), bbox_inches = 'tight', pad_inches = 0.1);close()

        for iter in range(2):
            clf()
            if iter == 0:
                vmin, vmax = None, None
                extrastr = '_freecolourscale'
            else: 
                vmin, vmax = -80., 80. #None, None
                extrastr = '_fixedcolourscale'
            for mask_iter in range(tot_masks):
                fsky = np.mean(mask_arr[mask_iter])
                H.mollview(currmap[0] * mask_arr[mask_iter], sub = (1,tot_masks,mask_iter+1), title_fontsize = 6, unit = r'$\mu K$', cbar_fontsize = 6, \
                    title = r'%s @ %s GHz + Mask %s: f$_{\rm sky} = %.2f$' %(dust_or_sync, nu, mask_iter, fsky), min = vmin, max = vmax); 
            savefig('%s/%s_%s%s.pdf' %(plfolder, dust_or_sync, nu, extrastr), bbox_inches = 'tight', pad_inches = 0.1);close()
        ...
        savefig('%s/S4_hitmaps.pdf' %(plfolder), bbox_inches = 'tight', pad_inches = 0.1);close()
        sys.exit()
